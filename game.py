...
class Objetspéciaux:    #classe des objet qui va donner leur stats et nom
    ...
    def __init__(self, nomObjet, valeur, stat):
...
while game:    # Boucle principale du jeu
    ...
    if commande == "exit" or commande == "quit" or commande == "quitter" or commande == "stop":
        ...
        if len(loupSolitaire.objetSpé) == 0:
            ...
            if question == "oui":
                if loupSolitaire.objetSpé[0].stat == "endurance":
                    loupSolitaire.maxEndurance -= loupSolitaire.objetSpé[0].valeur
                elif loupSolitaire.objetSpé[0].stat == "habileté":
                    loupSolitaire.habileté -= loupSolitaire.objetSpé[0].valeur
            ...
            for i in loupSolitaire.objetSpé:
                ...
                n += 1
            reponse = int(input())
            if loupSolitaire.objetSpé[reponse].stat == "endurance":
                loupSolitaire.maxEndurance -= loupSolitaire.objetSpé[reponse].valeur
            elif loupSolitaire.objetSpé[reponse].stat == "habileté":
                loupSolitaire.habileté -= loupSolitaire.objetSpé[reponse].valeur
